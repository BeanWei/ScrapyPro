...
class LagouSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield FormRequest(url=self.url, formdata=post_data, headers=self.headers, callback=self.parse_totalPn)
    ...
    def parse_totalPn(self, response):
        ...
        for pn in range(1, 3):
            ...
            yield FormRequest(url=self.url, formdata=post_data, headers=self.headers, callback=self.parse_totalUrl)
        ...
        if (res.get("success")):
            if res.get('content').get('positionResult').get('resultSize') != 0:
                ...
                for result in results:
                    ...
                    yield Request(url=item['url'], meta={'item_1': item}, headers=headers, callback=self.parse_detail)
